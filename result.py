...
class ResultClass:
    def __init__(self, root):
        ...
        #====button=====
        btn_add=Button(self.root,text='Submit',font=("times new roman",15),activebackground='lightgreen',cursor='hand2',command=self.add).place(x=300,y=420,width=120,height=35)
        btn_clear=Button(self.root,text='Clear',font=("times new roman",15),activebackground='lightgray',cursor='hand2',command=self.clear).place(x=430,y=420,width=120,height=35)

        #====Image=====
        logo_path = os.path.join(BASE_DIR, "Images", "result.jpg")
        self.logo = Image.open(logo_path)
        self.logo = self.logo.resize((500, 300), Image.LANCZOS)
        self.logo = ImageTk.PhotoImage(self.logo)
        self.lbl_logo=Label(self.root,image=self.logo).place(x=650,y=100)
